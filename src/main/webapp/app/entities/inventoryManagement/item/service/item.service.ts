...
type RestOf<T extends IItem | NewItem> = Omit<T, 'createdDate'> & {
...
export class ItemService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/items', 'inventorymanagement');
  ...
  protected convertDateFromClient<T extends IItem | NewItem | PartialUpdateItem>(item: T): RestOf<T> {
    return {
      ...item,
      createdDate: item.createdDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restItem: RestItem): IItem {
    return {
      ...restItem,
      createdDate: restItem.createdDate ? dayjs(restItem.createdDate) : undefined,
    };
  }
